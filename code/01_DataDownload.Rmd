...
```{r}
recs_temp <- recs_all %>%
  select(YEAR, HOTMA, COLDMA, TEMPMA) %>%
  group_by(YEAR) %>%
  summarise(HOTMA = sum(HOTMA), COLDMA = sum(COLDMA), TEMPMA = sum(TEMPMA))
```
...
recs_recode <- recs_all %>%
  ...
  mutate(APT = ifelse(TYPEHUQ >= 4, 1, 0)) %>%
...
recs_subset <- recs_recode %>%
  select(DBT1, DBT99, NONWHITE, OVER65, LIVESALONE, LARGEHOUSE, POVERTY, EMPLOYHH, HIGHSCHOOL, RENT, PAYSUTL, TYPEHUQ, YEARMADERANGE, WALLTYPE, ROOFTYPE, ADQINSUL, DRAFTY, WINDOWS, WINFRAME, TYPEGLASS, AIRCOND, HEATHOME, COOLTYPE, EQUIPM, ENRGINSEC, NUMCFAN, NUMFLOORFAN, OFFGRID, TEMPMA)
```
