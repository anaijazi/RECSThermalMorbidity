...
results_long <- results_resamples %>%
  ...
  filter(Sampling == "smote" & MLMethod == "knn") %>%
  mutate(Metric = factor(Metric, levels = c("Sens", "Spec", "ROC"))) # %>%
  # filter(Model == "All" | Model == "Building.All" | Model == "Demographic")

ggplot(results_long, aes(x= Model, y = Value, fill = Model)) +
  # geom_hline(yintercept = c(0, 0.5, 1), linetype = "dashed") +
  # ylim(0, 1) +
  # scale_fill_manual(values = c("#ee833d", "#59abb7", "#495893")) +
  geom_boxplot() +
