...
### Load data
```{r}
results_file1 <- "../data/results_compiled_20231125.csv"
results_file2 <- "../data/results_compiled_20231126.csv"

results1 <- read_csv(results_file1)
results2 <- read_csv(results_file2) 

results_compiled <- results1 %>%
  full_join(results2) %>%
...
best_all <- results_converged %>%
  filter(input_features == "All" & imbalance == "rose" & mlMethod == "multinom")
...
plot_BalancedAccuracy_input <- ggplot(results_bootstrapped, aes(x = Number, y = mean_BalancedAccuracy, fill = input_features, pattern = imbalance)) +
  ...
  annotate("rect", xmin = 54.5, xmax = 78.5, ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "#b2182b")
...
plot_BalancedAccuracy_combined <- ggarrange(plot_BalancedAccuracy_input, plot_BalancedAccuracy_best, ncol = 2, nrow = 1, widths = c(1,0.5), align = "hv")
```
...
plot_Recall_combined <- ggarrange(plot_Recall_input, plot_Recall_best, ncol = 2, nrow = 1, widths = c(1,0.5), align = "hv")
```
...
plot_Precision_combined <- ggarrange(plot_Precision_input, plot_Precision_best, ncol = 2, nrow = 1, widths = c(1,0.5), align = "v")
```

Combined plot
```{r}
ggarrange(plot_BalancedAccuracy_combined, plot_Recall_combined, plot_Precision_combined, ncol = 1, nrow = 3, align = "hv", common.legend = TRUE)
...
ggarrange(plot_BalancedAccuracy_imbalance_box, plot_BalancedAccuracy_mlType_box, align = "hv", nrow = 2, ncol = 1, common.legend = TRUE)
