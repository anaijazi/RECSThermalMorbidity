...
```{r}
plot_varImp <- ggplot(varImp_bootstrapped, aes(x = reorder(LONG.NAME, -ORDER), y = mag_Coef, colour = input_features, group = input_features, alpha = nonzero)) +
  scale_colour_manual(values = rev(my_pal3)) +
  scale_alpha(range = c(0.3, 1)) +
  geom_hline(yintercept = 0, linewidth = 0.3, colour = "#b2182b") +
  geom_errorbar(aes(ymin = mag_Coef-(2*sd_Coef), ymax = mag_Coef+(2*sd_Coef)), width = 0.2, colour = "#000000", linewidth = 0.2) +
  geom_point(size = 2) +
  theme_custom() +
  coord_flip() +
...
ggsave("Figure5_coefficiencts.pdf", units = "in", width = 6.5, height = 4)
```
