...
plot_HOTMA <- plot_usmap(data = merged, values = "HOTMA_rate") +
  scale_fill_continuous(low = "#f7f7f7", high = "#b2182b", na.value = "#f7f7f7", label = percent, name = "Heat-related illness", limits = c(0, 0.03), breaks = seq(0, 0.03, by = 0.01)) +
  theme(legend.position = "top")
```
2020 state level estimates
```{r}
plot_COLDMA <- plot_usmap(data = merged, values = "COLDMA_rate") +
  scale_fill_continuous(low = "#f7f7f7", high = "#2166ac", na.value = "#f7f7f7", label = percent, name = "Cold-related illness", breaks = seq(0, 0.03, by = 0.01), limits = c(0, 0.03)) +
  theme(legend.position = "top") 
```
...
results_all <- results_2015 %>%
  ...
  mutate(label = case_when(variable == "TEMPMA" ~ "Any temperature-related illness",
                           variable == "HOTMA" ~ "Heat-related illness",
                           variable == "COLDMA" ~ "Cold-related illness"))
```
...
plot_total <- ggplot(results_all, aes(x = reorder(label,total), y = total, group = year, fill = year)) +
  ...
  annotate("text", x = "Heat-related illness", y = 2000000, label = "Total U.S. households\n2015: 118.2 M\n2020:123.5 M", size = 2) +
  theme_custom() +
  coord_flip() +
...
```{r}
plot_maps <- ggarrange(plot_COLDMA, plot_HOTMA, ncol = 2, nrow = 1)
